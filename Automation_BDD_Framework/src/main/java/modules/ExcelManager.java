...
public class ExcelManager {
	...
	public ExcelManager(String filename) {
		if(prop.getProperty("environment").equalsIgnoreCase("QA")) {
			ExcelManager.filename= System.getProperty("user.dir")+prop.getProperty("qaFilepath")+"\\"+filename;
		}else if (prop.getProperty("environment").equalsIgnoreCase("PROD")) {
			ExcelManager.filename= System.getProperty("user.dir")+prop.getProperty("prodFilepath")+"\\"+filename;
		}else {
	...
	public HashMap<String, String> dataRead(String sheetname, String TCid)
	{
		...
		try {
			...
			for (Entry<String, String> new_map : map.entrySet()) {
				clocount++;
